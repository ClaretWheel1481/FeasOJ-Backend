...
func InitializeContainerPool(n int) {
	...
	for i := 0; i < n; i++ {
		...
		if err != nil {
			log.Printf("[ContainerPool] Error starting container during preheat: %v", err)
			continue
		}
		containerPool <- containerID
	}
	log.Printf("[ContainerPool] Preheated %d containers", len(containerPool))
}

// AcquireContainer 从池中获取一个空闲容器（若池为空则阻塞等待）
func AcquireContainer() string {
	containerID := <-containerPool
	log.Printf("[ContainerPool] Acquired container %s", containerID)
	return containerID
}
...
func ResetContainerForPool(containerID string) error {
	...
	if err := resetCmd.Run(); err != nil {
		log.Printf("[ContainerPool] Error resetting container %s: %v", containerID, err)
		return err
	}
	return nil
}
...
func ReleaseContainer(containerID string) {
	...
	if err := ResetContainerForPool(containerID); err != nil {
		log.Printf("[ContainerPool] Reset failed for container %s: %v, terminating it", containerID, err)
		// 清理失败，则终止当前容器
		TerminateContainer(containerID)
		// 尝试启动一个新容器替换
		newContainerID, err := StartContainer()
		if err != nil {
			log.Printf("[ContainerPool] Failed to start new container: %v", err)
			// 启动失败，则直接返回，不归还任何容器
			return
		}
	...
	containerPool <- containerID
	poolMutex.Unlock()
	log.Printf("[ContainerPool] Released container %s back to pool", containerID)
}
...
func ShutdownContainerPool() {
	...
	for containerID := range containerPool {
		TerminateContainer(containerID)
		log.Printf("[ContainerPool] Terminated container %s", containerID)
	}
	poolMutex.Unlock()
}
